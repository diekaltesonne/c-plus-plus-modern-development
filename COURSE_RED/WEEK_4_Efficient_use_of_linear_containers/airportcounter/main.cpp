...
*/
/* Класс должен уметь конструироваться по списку аэропортов вылета,
...
public:
  ...
  AirportCounter(){
      size_t j = 0;
      for (auto& i: db) {
        i.first = static_cast<TAirport>(j);
        i.second = 0;
        j++;
      }
      //db = {};
...
//      //db = {};
      while(begin!=end){
          db[static_cast<size_t>(*begin)].second++;
          begin++;
      }
//      copy(begin,end,db);
  }
  ...
  size_t Get(TAirport airport) const{
      return db.at(static_cast<size_t>(airport)).second;
  }

  // добавить данный элемент
  void Insert(TAirport airport){
      db[static_cast<size_t>(airport)].second++;
  }
  ...
  void EraseAll(TAirport airport){
      db[static_cast<size_t>(airport)].second = 0 ;
  }
...
public:
  ...
  AirportCounter(TIterator begin, TIterator end){
      while(begin!=end){
          //db[static_cast<size_t>(*begin)]++;
          begin++;
      }
  }
  // получить количество элементов, равных данному
  size_t Get(TAirport airport) const{
      return db.at(static_cast<size_t>(airport));
  }
  ...
  Items GetItems() const{
      ...
      for(int i =0 ; i< db.size();i++){
          a[i] = {static_cast<TAirport>(i),static_cast<size_t>(db[i])};
      }
      return a;
  }

private:
    array<size_t,static_cast<size_t>(TAirport::Last_)> db= {0};

};

/*Набор возможных аэропортов фиксирован в виде enum и потому
 * не может изменяться в рамках одного сеанса работы системы статистики,
 * то есть в рамках одного экземпляра AirportCounter.
