...
Функцию FindNamesHistory, возвращающую по году и словарю «год → имя (фамилия)» всю историю изменений имени (фамилии) к этому году в виде вектора строк.
Функцию BuildJoinedName, по истории изменений имени (фамилии) возвращающую эту историю в виде единой строки: сначала последнее имя, затем в круглых скобках через запятую все предыдущие имена.
Функцию BuildFullName, по имени и фамилии (возможно, неизвестным, то есть пустым) возвращающую полное имя в требуемом формате. В прошлой задаче этот код находился непосредственно в методе GetFullName, теперь же он понадобился и в методе GetFullNameWithHistory — потому мы и выделили его в отдельную функцию.
