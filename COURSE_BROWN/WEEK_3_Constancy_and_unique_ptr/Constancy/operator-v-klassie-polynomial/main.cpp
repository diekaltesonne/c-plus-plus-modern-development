...
private:
      ...
      public:
          ...
          IndexProxy& operator= (const IndexProxy& other){
            ...
            IndexProxy& operator= (T x){
                if(degree_ >= poly_.coeffs_.size()){
                    poly_.coeffs_.resize((poly_.coeffs_.size()+ (degree_ - (poly_.coeffs_.size()-1))),0);
                }
                poly_.coeffs_[degree_] = x;
                poly_.Shrink();
                //cout<<poly_.coeffs_.size()<<"asd"<<endl;
                return *this;
            }
      private:
          Polynomial& poly_;
...
public:
      ...
      IndexProxy operator [](size_t degree_){
          return IndexProxy(*this,degree_);
...
void TestNonconstAccess() {
  ...
  std::cout <<"OK2" <<poly.Degree()<<endl;
  std::cout <<"OK3" <<poly[6]<<endl;
  std::cout <<"OK4" <<poly[7]<<endl;
  ASSERT_EQUAL(poly[6], 0);
  ASSERT_EQUAL(poly[608], 0);
  ASSERT_EQUAL(poly.Degree(), 5);
  poly[608] = 0;
  std::cout <<"OK3" <<poly.Degree()<<endl;
  ASSERT_EQUAL(poly.Degree(), 5);
  {
