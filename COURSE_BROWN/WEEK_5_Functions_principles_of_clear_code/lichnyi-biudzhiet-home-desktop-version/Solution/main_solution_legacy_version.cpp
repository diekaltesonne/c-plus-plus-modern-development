...
array<double, VERTEX_COUNT> tree_values_sp,tree_spend;
...
void Push_2(size_t v, size_t l, size_t r) {
  for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
    if (w < VERTEX_COUNT) {
      tree_spend[w] += tree_spend[v];
      tree_values_sp[w] += tree_spend[v] * (r - l) / 2;
    }
  }
  tree_spend[v] = 0;
}
...
double ComputeSum_2(size_t v, size_t l, size_t r, size_t ql, size_t qr) {
  if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
    return 0;
  }
  Push_2(v, l, r);
  if (ql <= l && r <= qr) {
    return tree_values_sp[v];
  }
  return ComputeSum_2(v * 2, l, (l + r) / 2, ql, qr)
      + ComputeSum_2(v * 2 + 1, (l + r) / 2, r, ql, qr);
}
...
int main() {
  ...
  for (int i = 0; i < q; ++i) {
    ...
    if (query_type == "ComputeIncome") {
      cout << ComputeSum(1, 0, DAY_COUNT_P2, idx_from, idx_to) - ComputeSum_2(1, 0, DAY_COUNT_P2, idx_from, idx_to)<< endl;
      //cout << ComputeSum_2(1, 0, DAY_COUNT_P2, idx_from, idx_to) << endl;
